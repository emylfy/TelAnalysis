...
Каждый таб обрамлён **recap'ом в стиле «Wrapped»** — текстовая сводка, sparkline активности, ключевые KPI (сообщения, участники, days active, медиа, минуты голосовых), юбилейные milestones и автокарточки highlights (пиковый час, самый громкий день, топ-эмодзи, самый длинный стрик…).

| Таб | Что в нём |
| --- | --- |
| **Overview** | Area-chart активности по дням, calendar heatmap по годам (по количеству или переключатель «писали-нет»), hour × weekday heatmap (подписи про пиковый час и долю ночи), overlap по часам для 1-на-1, сессии разговоров (количество, среднее число сообщений, суммарное время, самые длинные по длительности), разбивка медиа + статистика голосовых + топ доменов ссылок, топ эмодзи, распределение reply latency с отдельной аркой вопрос→ответ, рейтинг самых длинных монологов |
| **Network** | Интерактивный force-directed граф (drag / zoom / hover, толщина и стрелки рёбер по числу и направлению ответов, цвет — Louvain communities), «портрет чата» со структурными находками (хабы, мосты, магниты, централизация), таблица структурных ролей по участникам, глубина reply-цепочек. Для маленьких чатов — bar chart; nodes/edges экспортируются в CSV для Gephi |
| **Words** | Wordcloud (по всему чату или по юзеру) + топ слов (график и таблица), биграммы/триграммы, богатство лексикона (MTLD) по участникам, извлечение email-ов и телефонов |
| **Channel** | Broadcast-wordcloud и частотный анализ для каналов |
| **Per-user** | Выбираешь любого участника: персона-карточка с трейт-чипами, KPI-плитки (сообщения, доля последнего слова, слов на сообщение, доля вопросов, доля реплаев), radar «манеры речи» (вопросы / восклицания / CAPS / реплаи против среднего по чату), дневная timeline + hour × weekday heatmap, распределения по времени суток и длине сообщений, скорость ответа / reciprocity, стрики и молчания активности, доля инициатора разговоров, доля форвардов, характерные фразы, топ слов и эмодзи, любимые стикеры, отличающие слова для 1-на-1 (log-odds) и лидерборд мата (`попаданий / 100 сообщений`) |

Опциональный **sentiment-анализ** на `rubert-tiny2-russian-sentiment` добавляет sentiment-over-time арку, линии тона по участникам, разбивку по часам / дням недели и самые позитивные / негативные сообщения — на вкладках **Words** и **Per-user**.
...
Чтобы перегенерировать их (или поправить профили участников), запусти генератор:
...
Картинки дашборда в этом README сгенерированы, а не сняты руками — после изменений в UI просто перегенерируй их:
