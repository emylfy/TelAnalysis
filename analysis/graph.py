...
def render_matplotlib_png(graph: GraphData, out_path: str) -> bool:
    """Render the graph to a PNG file. Returns True on success.
    Skips draw if too many nodes."""
    GRAPH_DRAW_MAX_NODES = 500
    GRAPH_LABEL_MAX_NODES = 200

    if not graph.nodes or len(graph.nodes) > GRAPH_DRAW_MAX_NODES:
        return False
    ...
    G = nx.DiGraph()
    show_labels = len(graph.nodes) <= GRAPH_LABEL_MAX_NODES
    for node_id, label, weight in graph.nodes:
        G.add_node(label, weight=float(weight))
    for source, target, _ in graph.edges:
        # we drew nodes by label; skip edge if its endpoints aren't in the node set
        # (edge endpoints are ids, nodes are labels — need a mapping)
        pass

    # Map id -> label for edges
    id_to_label = {nid: label for nid, label, _ in graph.nodes}
    for source, target, _ in graph.edges:
        s_lbl = id_to_label.get(source)
        t_lbl = id_to_label.get(target)
        if s_lbl and t_lbl and s_lbl in G.nodes and t_lbl in G.nodes:
            G.add_edge(s_lbl, t_lbl, weight=1.3)
