...
# Bundled demo exports (generated by tools/gen_demo_data.py) — a group chat and
# a 1-on-1. Each onboarding button appears only when its file is present on disk.
_DEMO_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "demo")
DEMO_PATH = os.path.join(_DEMO_DIR, "group_demo.json")
DEMO_PATH_PERSONAL = os.path.join(_DEMO_DIR, "personal_demo.json")


def _ingest_upload(upload) -> None:
    """Persist an uploaded export to a temp file, mark it loaded, rerun."""
    ...
    tmp.close()
    st.session_state["loaded_path"] = tmp.name
    st.session_state["loaded_label"] = upload.name
    st.rerun()


def _ingest_path(path: str, label: str | None = None) -> None:
    """Mark a filesystem path (file or export dir) as loaded, rerun."""
    st.session_state["last_path"] = path
    st.session_state["loaded_path"] = path
    st.session_state["loaded_label"] = label or os.path.basename(path.rstrip("/")) or path
    st.rerun()


def _select_export(path: str, label: str | None = None) -> None:
    """on_click callback for the demo buttons — mark the export loaded; Streamlit
    reruns into the dashboard after the callback. (The onboarding container is
    torn down on the dashboard side, see the empty `tla_onboarding` re-declare.)
    """
    st.session_state["last_path"] = path
    st.session_state["loaded_path"] = path
    st.session_state["loaded_label"] = label or os.path.basename(path.rstrip("/")) or path
...
# Scoped styling for the onboarding card. Covers: centring the fixed-width
# container; neutral kbd-style menu-path chips (the theme's code colour is green,
# which reads as "success" — wrong for breadcrumbs); tighter dividers; and hiding
# Streamlit's built-in *English* file-uploader strings (we relabel in RU/EN
# ourselves — the browse-button label is injected per-language at render time).
_ONBOARDING_CSS = """
<style>
/* Streamlit puts width=460 on a stLayoutWrapper around the keyed container; that
   wrapper is the flex item that must be centred (align-items:stretch pins it left). */
[data-testid="stLayoutWrapper"]:has(> .st-key-tla_onboarding) {
    align-self: center !important;
    margin-left: auto !important;
    margin-right: auto !important;
}
.st-key-tla_onboarding code {
...
.st-key-tla_onboarding hr { margin: 0.3rem 0 0.5rem; }
/* Collapse the empty dropzone box down to just a full-width button (you can't
   paste a path here — that's the text field below; drag-drop still works on the
   button). Hide Streamlit's instructions and zero the button's English label
   (it lives in a child with its own font-size; :not(svg) keeps any SVG icon),
   then ::after re-adds our localized label. */
.st-key-tla_onboarding [data-testid="stFileUploaderDropzoneInstructions"] { display: none; }
.st-key-tla_onboarding [data-testid="stFileUploaderDropzone"] {
    border: none;
    background: transparent;
    padding: 0;
    min-height: 0;
}
.st-key-tla_onboarding [data-testid="stFileUploaderDropzone"] button {
    width: 100%;
    height: 2.5rem;
    justify-content: center;
    font-size: 0;
}
/* Hide Streamlit's own button content (its icon is a font glyph that fights our
   centring) and draw our own icon + label via ::after — fully under our control. */
.st-key-tla_onboarding [data-testid="stFileUploaderDropzone"] button > * { display: none; }
</style>
"""


def _render_onboarding() -> None:
    """Empty-state shown until an export is loaded.

    A fixed-width, centred card so the primary action (load a file) sits next to
    its guidance instead of being split across the sidebar and a void.

    The card is a *keyed* container: the key gives it a stable identity so that
    on the load→dashboard rerun Streamlit removes it cleanly instead of
    reconciling it against the dashboard's top-level layout (which previously
    reparented these widgets into the period filter's column).
    """
    st.markdown(_ONBOARDING_CSS, unsafe_allow_html=True)
    # Onboarding is a full-screen welcome — hide the (otherwise empty) sidebar so
    # the rail doesn't read as unfinished. The language toggle moves into the card
    # below; the sidebar returns once an export is loaded.
    st.markdown(
        "<style>"
        "section[data-testid='stSidebar'],"
        "[data-testid='stSidebarCollapsedControl']{display:none !important;}"
        ".st-key-onb_lang{position:fixed;top:.85rem;right:1.5rem;z-index:1000;width:auto;}"
        ".st-key-onb_lang div[role='radiogroup']{flex-direction:row !important;gap:.4rem;}"
        "</style>",
        unsafe_allow_html=True,
    )
    # Browse-button label is language-specific, so inject it as ::after content
    # (the static CSS above zeroes the button's own English text).
    _browse = i18n.t("Выбрать файл")
    st.markdown(
        '<style>.st-key-tla_onboarding [data-testid="stFileUploaderDropzone"] '
        f'button::after {{ content: "↑  {_browse}"; font-size: 0.875rem !important; }}</style>',
        unsafe_allow_html=True,
    )
    with st.container(key="tla_onboarding", width=460):
        # Language toggle — pinned to the screen's top-right (sidebar is hidden).
        with st.container(key="onb_lang"):
        ...
        st.markdown(
            "<div style='text-align:center; margin-top:5vh; margin-bottom:1.5rem'>"
            + ui_components.logo_svg(56)
            + "<h1 style='margin:.5rem 0 .15rem; font-size:2rem; letter-spacing:-0.02em'>"
            "TelAnalysis</h1>"
            "<p style='opacity:.55; margin:0; font-size:.95rem'>"
            f"{i18n.t('Локальная аналитика Telegram-переписок — данные не покидают устройство')}</p>"
            "</div>",
            unsafe_allow_html=True,
        )

        # Load your own: paste a path (left) or pick a file (browse button, right)
        # on one row. Both load the export; the file uploader's dropzone is
        # CSS-collapsed to just its button and bottom-aligned with the field.
        col_path, col_pick = st.columns([3, 2], vertical_alignment="bottom")
        with col_path:
            path_input = st.text_input(
                i18n.t("Путь к файлу или папке экспорта"),
        ...
        with col_pick:
            upload = st.file_uploader(
                i18n.t("Перетаскивание экспорта сюда или выбор файла"),
                type=["json", "html"],
                key="onb_uploader",
                label_visibility="collapsed",
            )
            if upload is not None:
                _ingest_upload(upload)
        if path_input:
            if os.path.exists(path_input):
                _ingest_path(path_input)
            else:
                st.error(i18n.t("Файл не найден"))
        st.caption(i18n.t("Путь — без лимита по размеру, быстрее для больших архивов."))

        with st.expander(i18n.t("Как получить экспорт?")):
            st.markdown(
                i18n.t(
                    "В **Telegram Desktop**:\n\n"
                    "- **Один чат** — `Настройки → Экспорт переписки`\n"
                    "- **Весь аккаунт** — `Настройки → Продвинутые настройки → Экспорт данных Telegram`\n\n"
                    "В окне экспорта нужно выбрать формат **JSON**. HTML тоже работает "
                    "(папка или `messages.html`), но менее точен."
                )
            )

        # Demo — secondary path, de-emphasized with a quiet caption below the
        # import help. Kept at the onboarding's TOP LEVEL (not inside an
        # expander): a button click *inside* an expander leaves a ghost copy of
        # that expander on the page after the load→dashboard rerun.
        _demos = [
            (DEMO_PATH, i18n.t("Групповой чат"), i18n.t("Демо · групповой чат"), "onb_demo_group"),
            (DEMO_PATH_PERSONAL, i18n.t("Личный чат"), i18n.t("Демо · личный чат"), "onb_demo_personal"),
        ]
        _demos = [d for d in _demos if os.path.exists(d[0])]
        if _demos:
            st.caption(i18n.t("Посмотреть на демо-данных:"))
            for _col, (_path, _label, _loaded, _key) in zip(st.columns(len(_demos)), _demos):
                _col.button(
                    "▶  " + _label,
                    width="stretch",
                    key=_key,
                    on_click=_select_export,
                    args=(_path, _loaded),
                )
...
# Resolve the loaded export first, so the sidebar renders ONLY on the dashboard.
# On the onboarding screen there's nothing to put in the rail, so we skip it
# entirely (and hide it via CSS) instead of showing an empty sidebar — the
# language toggle moves into the welcome card. State lives in
# st.session_state["loaded_path"] / ["loaded_label"].
...
json_path: str | None = loaded_path

# Sidebar removed — brand, language, "load other file" and all chat/period
# controls now live in the top context bar (see "Top context bar" below).

if json_path is None:
    _render_onboarding()
    st.stop()

# Re-declare the onboarding container EMPTY (same key) so Streamlit tears its
# children down immediately on the load→dashboard rerun. Without this, a ghost of
# the onboarding (the demo buttons) lingers over the dashboard for the whole
# heavy render (~5s) until reconciliation finishes.
st.container(key="tla_onboarding")

src_mtime = os.path.getmtime(json_path)
data = ui_cache.load_data(json_path, src_mtime)
chats = loader.list_chats(data)
is_html_source = data.get("_source") == "html"

# --- Top context bar (replaces the left sidebar) ---------------------------
# Brand + chat switcher on the left; period, language and "load other file" on
# the right; content runs full-width below. Sticky — see .st-key-tla_topbar.
date_range = None
with st.container(key="tla_topbar"):
    # Controls use the bar's width (brand left, then chat · period spread,
    # language + reset on the right).
    _b_brand, _b_chat, _b_period, _b_lang, _b_reset = st.columns(
        [2.4, 3.0, 2.6, 1.3, 0.7], vertical_alignment="center"
    )
    with _b_brand:
        st.markdown(
            '<div class="tla-brand">'
            + ui_components.logo_svg(24)
            + '<span class="tla-brand-name">TelAnalysis</span></div>',
            unsafe_allow_html=True,
        )

    # Chat selection. A single-chat export needs no switcher (the hero names it);
    # archives get ONE searchable picker. Its first option combines all chats, so
    # the old type-filter + combine-checkbox + picker trio collapses into one
    # control (type still readable in each option's label, search covers the rest).
    with _b_chat:
        if len(chats) == 1:
            chat = chats[0]
        else:
            _COMBINE = "__combine__"
            # Group the list by chat type (personal → group → channel → bot →
            # saved → other), alphabetical by name within each type. The list
            # clusters by type and stays predictable; search still matches names.
            _TYPE_RANK = {
                "personal_chat": 0,
                "private_group": 1,
            ...
            _order = sorted(
                range(len(chats)),
                key=lambda i: (_TYPE_RANK.get(chats[i].type, 9), chats[i].name.lower()),
            )
            _options = [_COMBINE, *_order]

            def _chat_opt(o):
                if o == _COMBINE:
                    return f"★ {i18n.t('Все чаты')} · {i18n.n_chats(len(chats))}"
                c = chats[o]
                return f"{c.name} · {i18n.chat_type_label(c.type)}"

            # Default: ?chat= from URL, else the first chat (index 1, not combine).
            _qp = st.query_params.get("chat")
            _default = 1 if len(_options) > 1 else 0
            if _qp == _COMBINE:
                _default = 0
            elif _qp:
                for _i, _c in enumerate(chats):
                    if str(_c.id) == str(_qp):
                        _default = _options.index(_i)
                        break
            _sel = st.selectbox(
                i18n.t("Чат"),
                options=_options,
                format_func=_chat_opt,
                index=_default,
                label_visibility="collapsed",
            )
            if _sel == _COMBINE:
                chat = loader.combined_chat(chats)
                chat.name = i18n.t("Все чаты")  # override English default name
                st.query_params["chat"] = _COMBINE
            else:
                chat = chats[_sel]
                st.query_params["chat"] = str(chat.id)

    # Period filter — popover whose label reflects the current range.
    ...
    with _b_period:
        if bounds is not None:
            # Date input lives directly in the bar (no popover wrapper): a
            # date_input *inside* a popover opens its calendar as a second
            # floating layer that overlaps the popover panel. One layer here.
    ...
    with _b_reset:
        if st.button("↻", help=i18n.t("Загрузить другой файл"), key="tla_reset"):
            st.session_state.pop("loaded_path", None)
            st.session_state.pop("loaded_label", None)
...
# HTML-export caveat (was a sidebar warning) — a slim note under the bar.
if is_html_source:
    st.warning(
        i18n.t(
...
# Cache key — change identity when the file content (mtime), chat or date range
# changes so all derived caches invalidate. Without mtime, regenerating an export
# under the same path (e.g. the bundled demo) serves stale per-day/KPI results.
cache_key = f"{json_path}::{src_mtime}::{chat.id}::{chat.type}::{from_d}::{to_d}"
...
if kpis.total_messages == 0:
    st.title(chat.name)
    st.warning(i18n.t("Нет сообщений в выбранном периоде."))
    st.stop()
...
# "Моменты" tab is hidden until reworked — its body is currently a placeholder
# ("таб переделывается"), and shipping an empty tab is worse than not showing it.
# Re-enable by restoring the append once the story-timeline lands (see the
# `elif key == "highlights"` branch below).
...
for tab, (_, key) in zip(tabs, tab_specs):
    with tab:
        if key == "overview":
            ...
            # Section 1: «Как часто» — timeline + calendar. Clicking a point on
            # the timeline drills down to that day's stats below, via the native
            # st.plotly_chart on_select (Streamlit ≥1.35). The old third-party
            # streamlit-plotly-events component bundled an ancient plotly.js that
            # mis-rendered this single-series area as a cumulative ramp.
            ...
            if per_day:
                ...
                # px.area tags the trace with a stackgroup; for a single series that
                # adds nothing but trips stacking code paths — drop it, fill to zero.
                fig.update_traces(
                    stackgroup=None,
                    fill="tozeroy",
                    line=dict(width=1.5, color=theme_mod.PALETTE["primary"]),
                    fillcolor="rgba(91, 143, 249, 0.15)",
                )
                ...
                _pts = (ev.get("selection", {}).get("points", []) if ev else []) or []
                if _pts:
                    st.session_state["selected_day"] = pd.to_datetime(
                        _pts[0]["x"]
                    ).strftime("%Y-%m-%d")

                # Affordance: the timeline is click-to-drill, but nothing signals
                # it. Show a one-time hint until a day is selected (after which the
                # drill-down panel itself is the cue).
                if "selected_day" not in st.session_state:
                    st.caption(i18n.t("👆 Клик по точке на графике раскрывает день."))
                # Insight caplets merged into one line to cut vertical caption noise.
                _ov_caps = [
                    highlights_mod.caplet_peak_day(per_day),
                    highlights_mod.caplet_peak_week(per_day),
                ]
                _ov_caps = [c for c in _ov_caps if c]
                if _ov_caps:
                    st.caption("  ·  ".join(_ov_caps))
                ...
                st.info(i18n.t("Нет сообщений с датами."))
            ...
            if any(any(row) for row in grid):
                ...
                _hr_caps = [
                    highlights_mod.caplet_peak_hour(grid),
                    highlights_mod.caplet_night_share(grid),
                ]
                _hr_caps = [c for c in _hr_caps if c]
                if _hr_caps:
                    st.caption("  ·  ".join(_hr_caps))
                ...
                if len(user_hours) == 2:
                    ...
                    st.caption(
                        i18n.t(
                            ...
                            "Бары нормализованы по пользователям — справедливо при разной активности."
            ...
            if lat.overall_seconds:
                ...
                lat_c1.metric(
                    i18n.t("Половина ответов — быстрее"),
                    latency_mod.humanize_seconds(lat.median_seconds),
                    help=i18n.t(
                        "Серединное время ответа: половина ответов быстрее этого "
                        "значения, половина — медленнее. Медиана, а не среднее, "
                        "чтобы редкие очень поздние ответы не искажали цифру."
                    ),
                )
                lat_c2.metric(
                    i18n.t("90% ответов — быстрее"),
                    latency_mod.humanize_seconds(lat.p90_seconds),
                    help=i18n.t(
                        "90-й перцентиль: 90% ответов укладываются в это время, "
                        "и только самые медленные 10% — дольше. Это «почти худший» "
                        "случай, а не среднее."
                    ),
                )
                lat_c3.metric(
                    i18n.t("Ответов учтено"),
                    f"{len(lat.overall_seconds):,}",
                    help=i18n.t(
                        "Сколько ответов вошло в расчёт времени выше. "
                        "Ответы дольше суток в расчёт не входят."
                    ),
                )
                ...
                if lat.dropped_over_cap:
                    ...
                    st.caption(
                        i18n.t(
                            "{n} ответов (>{h}ч) не вошли в расчёт — это {p}% всех пар."
                        ).format(
                ...
                if lat.qa_seconds:
                    ...
                    st.markdown(f"**{i18n.t('Ответы на вопросы (на сообщения с «?»)')}**")
                    qa_c1, qa_c2, qa_c3 = st.columns(3)
                    qa_c1.metric(
                        i18n.t("Половина ответов — быстрее"),
                        latency_mod.humanize_seconds(lat.qa_median_seconds),
                        help=i18n.t(
                            "Половина ответов на вопросы — быстрее этого значения. "
                            "По сравнению с ответами на все сообщения: {d}."
                    ...
                    qa_c2.metric(
                        ...
                        help=i18n.t(
                            "90-й перцентиль времени ответа на сообщения с вопросом "
                            "(«?»): 90% таких ответов быстрее этого значения."
                        ),
                    )
                    qa_c3.metric(
                        i18n.t("Вопросов с ответом"),
                        f"{len(lat.qa_seconds):,}",
                        help=i18n.t("Сколько вопросов получили ответ и вошли в расчёт времени выше."),
                    )
            ...
            if mono_stats.longest:
                ...
                st.caption(
                    i18n.t(
                        "Подряд N+ сообщений от одного пользователя без ответа другого. "
            ...
            if not g.nodes:
                st.info(i18n.t("В этом чате нет участников (только сервисные события?)."))
            else:
                ...
                if summary:
                    ...
                    fig_int = px.bar(
                        ...
                        barmode="group",
                        template="telanalysis",
                        title=i18n.t("Кто пишет, кто отвечает"),
                    )
                ...
                if len(g.nodes) <= 3:
                    ...
                    with st.spinner(i18n.t("Построение интерактивного графа…")):
                    ...
                    if html:
                        ...
                        st.caption(
                            i18n.t(
                                "Перетаскивание узлов · колесо — зум · наведение для деталей. "
                                "Рёбра объединены по числу ответов, толщина ~ частоте. "
                                "Цвета — сообщества (Louvain modularity)."
                            )
                        )
                    else:
                        st.warning(
                            i18n.t(
                                "Граф слишком большой для интерактивного рендера "
                                "({n} узлов). CSV для Gephi — ниже."
                            ).format(n=len(g.nodes))
                        )

            with st.expander(i18n.t("Скачать CSV (для Gephi)")):
            ...
            cwords1.metric(i18n.t("Проанализировано пользователей"), f"{len(res.users):,}")
            ...
            if res.sentiment_available and res.dated_scores:
                with st.expander(i18n.t("Сентимент во времени")):
                    ...
                    # Sentiment caveat is stated once at the top of the tab
                    # (chat-average caption) — not repeated here, to cut noise.
            ...
            if res.users and any(u.total_tokens > 0 for u in res.users.values()):
                ...
                # (MTLD explanation moved into the section header's help tooltip.)
                _df(
                    voc_df,
                    width="stretch",
                    hide_index=True,
            ...
            if not participants:
                ...
                if pu_per_day:
                    ...
                    # See overview timeline: drop stackgroup (+ explicit fill) so a
                    # single series renders as a per-day area, not a cumulative ramp.
                    fig_pu.update_traces(
                        stackgroup=None,
                        fill="tozeroy",
                        line=dict(width=1.5, color=theme_mod.PALETTE["primary"]),
                        fillcolor="rgba(91, 143, 249, 0.15)",
                ...
                with col_b:
                    ...
                    if user_lats:
                        ...
                        st.caption(
                            i18n.t("Половина быстрее {m} · 90% быстрее {p} · {n}").format(
            ...
            st.info(
                i18n.t(
                    ...
                    "по эпохам — пока доступны Обзор и По участникам."
                )
            )
